...
def post_liver_data():
    ...
    TB = float(request.json.get('TB'))
    DB = float(request.json.get('DB'))
    Alkphos = int(request.json.get('Alkphos	'))
    Spgt = int(request.json.get('Sgpt'))
    Sgot = int(request.json.get('Sgot'))
    TP = float(request.json.get('TP'))
    ALB = float(request.json.get('ALB'))
    AG_Ratio = float(request.json.get('A/G Ratio'))
    print(Age)
    df3 = pd.DataFrame(
        {
            'Age': [Age],
